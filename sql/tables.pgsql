...
@column retry_index             The number of previous attempts of this job
@column status                  state the job is in
...
CREATE TABLE attempt (
    ...
    UNIQUE (job_id, retry_index)                                   -- to avoid repeating tasks
);
CREATE INDEX ON attempt (role_id);                                 -- for finding the attemps linked to a role (not used by the current code)
